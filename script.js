...
//let json = require('./bible/3John.json');
//log(json.chapters[0].verses[0]);
...
function updateText2() {
    ...
    var version = document.getElementById('translation').value;
    verses = bible[getBook(chapterE)].chapters[chapter.value];
    var output = '';
    for (x = 1; x > verses; x++) {
        getVerses(bibleBook + ' ' + bibleChapter + ' ' + x, version);
        setTimeout(function() {
            script = document.getElementById('script').innerHTML;
            output = output + '<p><sub>' + x + '</sub>' + script;
        }, 300);
    }
